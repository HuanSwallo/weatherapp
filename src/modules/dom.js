...
export function loadWeatherData(payload, unit) {
  ...
  // Icon
  const mainIcon = document.querySelector(".main-bottom i");
  const iconCode = currentData.weather[0].icon;
  const iconClass = mapIconToWeatherIcon(iconCode);

  mainIcon.className = `wi ${iconClass}`;

  // Temperatures and description
  document.querySelector(".temp").textContent = `${Math.round(
    currentData.main.temp
  )}°`;
  document.querySelector(".highlowtemp").textContent = `${Math.round(
  ...
  document.getElementById("sunrise-output").textContent = new Date(
    currentData.sys.sunrise * 1000
  ).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });
  document.getElementById("sunset-output").textContent = new Date(
    currentData.sys.sunset * 1000
  ).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });

  // Rain probability: use today's max PoP from daily summary if available
  const today = daily?.[0];
  document.getElementById("rain-output").textContent = today
    ? `${Math.round(today.pop * 100)}%`
    : "N/A";

  // Next days cards
  loadNextDays(daily, unit);
}

// 
function loadNextDays(daily) {
  const items = document.querySelectorAll("#nextdays li");

  // daily includes today + next days.
  const next = (daily ?? []).slice(0, 5);
...
// Load city suggestions into the suggestions list
